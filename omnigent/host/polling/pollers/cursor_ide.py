"""Cursor IDE ambient transcript poller."""
...
from omnigent.host.polling.pollers.cursor_config import load_cursor_ide_poller_config
from omnigent.host.polling.pollers.cursor_ide_local import CursorIdeLocalSubPoller
from omnigent.host.polling.pollers.cursor_ide_remote import CursorIdeRemoteSubPoller
from omnigent.ssh_connections_store import SshConnectionProfile


class CursorIdeAmbientPoller(AmbientPoller):
    """Mirror standalone and remote Cursor IDE transcripts into Omnigent."""
    ...
    def _load_config(self) -> AmbientPollerConfig:
        config = load_cursor_ide_poller_config(self._config_path)
    ...
    async def _hydrate_state(self, ctx: PollContext) -> AmbientBridgeState:
        return await hydrate_ambient_bridge_state(
            ctx.client,
            host_id=ctx.host_id,
            import_source="cursor-ide",
        )

    def _empty_state(self) -> AmbientBridgeState:
        return AmbientBridgeState(tracks={})

    def _create_local_subpoller(self) -> CursorIdeLocalSubPoller:
        return CursorIdeLocalSubPoller(projects_root=self._projects_root)
    ...
    def _create_remote_subpoller(
        ...
        *,
        interval_s: float,
        backoff_cap_s: float,
    ) -> CursorIdeRemoteSubPoller:
        return CursorIdeRemoteSubPoller(
            profile,
