"""Local filesystem Cursor CLI chat polling."""
...
class CursorCliLocalSubPoller(LocalAmbientSubPollerBase):
    """Scan ``~/.cursor/chats`` on the host machine."""

    def __init__(self, *, chats_root: Path | None = None) -> None:
        super().__init__()
        self._chats_root = chats_root or default_cursor_chats_root()
    ...
    def _load_import(self, discovery) -> LocalSessionImport:
        return load_cursor_cli_session(discovery.store_path)

    def _initial_byte_offset(self, discovery, imported: LocalSessionImport) -> int:
        return initial_cursor_cli_rowid(discovery.store_path)

    def _read_updates(self, tracked: TrackedAmbientSession) -> AmbientReadResult:
        read_result = read_cursor_cli_from_rowid(
            Path(tracked.source_path),
            last_rowid=tracked.byte_offset,
        )
