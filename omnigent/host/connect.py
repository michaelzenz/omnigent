...
class HostProcess:
    ...
    async def _handle_launch(
        ...
        if (
            ...
            and not await asyncio.to_thread(_harness_now_configured, frame.harness)
                ...
                if builtin is not None:
                    ...
                    if not config_path.exists():
                        ...
                        config_path.write_text(
                            yaml.safe_dump(builtin.default_config, sort_keys=False)
                        )
        ...
        try:
            ...
            with self._host_subprocess_op():
                if frame.auto_reuse:
                    ...
                    # into a fresh worktree (deleted-worktree recreate).
                    created = await asyncio.to_thread(
                        create_worktree_streaming,
                        repo_path=frame.repo_path,
                        branch_name=frame.branch_name,
    ...
    async def _handle_worktree_sizes(
        ...
        if cached is not None and not frame.force:
            ...
            if cache.needs_recalc(frame.repo_path) and cache.mark_in_flight(frame.repo_path):
                _ = _asyncio.create_task(self._background_worktree_recalc(frame.repo_path))
