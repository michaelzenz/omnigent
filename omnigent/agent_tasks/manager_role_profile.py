"""Load manager glossary profiles for task bootstrap."""
...
def load_manager_role_profile(
    ...
    """Return the glossary profile selected by ``task.manager_role_key``."""
    return task_role_profile_store.get(owner_user_id, task.manager_role_key)
...
def get_or_create_manager_role_profile(
    *,
    ...
    """Load or auto-provision the manager glossary profile for ``task``."""
    existing = load_manager_role_profile(task_role_profile_store, owner_user_id, task)
