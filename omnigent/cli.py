...
def _display_db_uri(db_uri: str) -> str:
    ...
    :param db_uri: The resolved store DB URI, e.g.
        ``"postgresql://<user>:<password>@db:5432/omnigent"``.
...
def server(
    ...
    if model_settings.policy_model is None:
        ...
        raw_llm: dict[str, object] = (
            dict(cfg.get("llm")) if isinstance(cfg.get("llm"), dict) else {}
        )
        ...
        if not raw_llm.get("profile"):
            ...
            if auth and auth.get("type") == "databricks" and isinstance(profile, str):
                raw_llm["profile"] = profile
