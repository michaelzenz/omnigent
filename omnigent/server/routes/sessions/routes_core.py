...
def register_core_routes(
    ...
    *,
    ...
    async def create_session(
        ...
        # A new worktree is launched by its background creation task once
        # the final workspace is known.
        _pending_worktree = (
            body.git is not None and not body.git.existing_worktree and body.host_id is not None
        )
    ...
    async def _create_bundled_session_from_multipart(
        ...
        if parsed_metadata.host_id is not None:
            ...
            workspace_validation = await validate_uploaded_bundle_host_workspace(
                user_id=user_id,
            ...
            parsed_metadata = parsed_metadata.model_copy(
                update={"workspace": workspace_validation.canonical_path}
            )
    ...
    async def update_session(
        ...
        if set_workspace:
            ...
            workspace_agent_id = conv_for_workspace.agent_id
            if workspace_agent_id is None:
                raise OmnigentError(
                    "session has no bound agent",
                    code=ErrorCode.NOT_FOUND,
                )
            agent_for_workspace = agent_store.get(workspace_agent_id)
            if agent_for_workspace is None:
                raise OmnigentError(
                    f"agent {workspace_agent_id!r} not found",
                    code=ErrorCode.NOT_FOUND,
                )
