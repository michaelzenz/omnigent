...
async def _maybe_adopt_session(session_id: str) -> None:
    ...
    try:
        ...
        if conv is None or conv.kind == "sub_agent":
            return
        # System role sessions (broker, secretary) must not be adopted as
        # orphans — their turns are triggered by the queue dispatcher, not a
        # user, so feeding them back into adoption creates a self-perpetuating
        # loop.
...
def _publish_status(
    ...
    if _queue_status_feed is not None:
        status_feed_task = asyncio.create_task(_queue_status_feed.notify(session_id, status))
        status_feed_task.add_done_callback(
    ...
    if status == "idle":
        ...
        with contextlib.suppress(RuntimeError):
            adoption_task = asyncio.create_task(_maybe_adopt_session(session_id))
...
async def _stop_session_host_runner(
    ...
    *,
    ...
    :param missing_ok: Ask the host to treat an already-missing runner as a
        successful stop.
...
def _compact_lock(session_id: str) -> asyncio.Lock:
    ...
    Return the process-local lock for explicit compaction.
    """
    return _compact_lock_impl(session_id)
