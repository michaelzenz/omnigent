...
class SessionGitOptions(BaseModel):
    ...
    def _check_existing_worktree(self) -> SessionGitOptions:
        ...
        if self.existing_branch and self.auto_create:
            raise ValueError("existing_branch and auto_create cannot both be true")
        if self.existing_branch and self.auto_fetch_base:
            raise ValueError("auto_fetch_base cannot be set when existing_branch is true")
