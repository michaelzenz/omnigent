...
TOOLS: list[ToolCatalogEntry] = [
    # Skills
    ToolCatalogEntry(
        "load_skill", "Load skills", "Load a skill's instructions by name.", "skills"
    ),
    ToolCatalogEntry(
        "read_skill_file",
        "Read skill files",
        "Read supporting files from an installed skill.",
        "skills",
    ),
    ToolCatalogEntry(
        "update_skill", "Update skills", "Update text files in an existing skill.", "skills"
    ),
    ...
    ToolCatalogEntry(
        "upload_file",
        "Upload files",
        "Upload a file from the workspace for download.",
        "files_content",
    ),
    ToolCatalogEntry(
        "list_files", "List files", "List uploaded or created files.", "files_content"
    ),
    ToolCatalogEntry(
        "download_file",
        "Download files",
        "Download a file by its ID to the workspace.",
        "files_content",
    ),
    ToolCatalogEntry(
        "serve_html",
    ...
    ToolCatalogEntry(
        "export_agent",
        "Export agents",
        "Copy a generated agent directory to a target path.",
        "files_content",
    ),
    # Agents & sessions
    ToolCatalogEntry(
        ...
        "Spawn a child session from an agent.",
        "agents_sessions",
    ),
    ToolCatalogEntry(
        "sys_session_send",
    ...
    ToolCatalogEntry(
        "sys_session_close",
        "Close sub-agent sessions",
        "Tombstone a sibling sub-agent session.",
        "agents_sessions",
    ),
    ToolCatalogEntry(
        "sys_session_list",
        "List sessions",
        "List sessions in two views: sub-agents and global.",
        "agents_sessions",
    ),
    ToolCatalogEntry(
        "sys_session_get_history",
        "Read session history",
        "Read recent items from a session's conversation.",
        "agents_sessions",
    ),
    ToolCatalogEntry(
        "sys_session_get_info",
        "Get session info",
        "Return a session's metadata and lifecycle status.",
        "agents_sessions",
    ),
    ToolCatalogEntry(
        "sys_session_share",
        "Share sessions",
        "Grant a user access to a session.",
        "agents_sessions",
    ),
    ToolCatalogEntry(
        "sys_session_rename",
        "Rename sessions",
        "Rename the current session with a short title.",
        "agents_sessions",
    ),
    ToolCatalogEntry(
        "sys_agent_list",
        "List available agents",
        "List launchable agents across built-in, session, and local configs.",
        "agents_sessions",
    ),
    ToolCatalogEntry(
        "sys_agent_get",
        "Get agent metadata",
        "Return the agent metadata bound to a session.",
        "agents_sessions",
    ),
    ToolCatalogEntry(
        "sys_agent_download",
    ...
    ToolCatalogEntry(
        ...
        "Launch a named terminal (tmux session).",
        "computer_terminal",
    ),
    ToolCatalogEntry(
        "sys_terminal_send",
        "Send to terminals",
        "Send text and/or key strokes to a running terminal.",
        "computer_terminal",
    ),
    ToolCatalogEntry(
        "sys_terminal_read",
        "Read terminal output",
        "Capture the visible pane plus optional scrollback.",
        "computer_terminal",
    ),
    ToolCatalogEntry(
        "sys_terminal_list",
        "List terminal sessions",
        "List the conversation's active terminal sessions.",
        "computer_terminal",
    ),
    ToolCatalogEntry(
        "sys_terminal_close",
    ...
    # Legacy File Interaction
    ToolCatalogEntry(
        "sys_os_read",
        "Read files (legacy)",
        "Read a text file from the OS environment.",
        "legacy_os_interaction",
    ),
    ToolCatalogEntry(
        "sys_os_write",
        "Write files (legacy)",
        "Write full contents of a text file in the OS environment.",
        "legacy_os_interaction",
    ),
    ToolCatalogEntry(
        "sys_os_edit",
    ...
    ToolCatalogEntry(
        "sys_cancel_async",
        "Cancel async tasks",
        "Cancel a previously dispatched async task.",
        "automation",
    ),
    ToolCatalogEntry(
        "sys_read_inbox",
    ...
    ToolCatalogEntry(
        ...
        "Cancel a running background task by task ID.",
        "automation",
    ),
    ToolCatalogEntry(
        "sys_timer_set", "Set timers", "Schedule a timer that fires after a delay.", "automation"
    ),
    ToolCatalogEntry(
        "sys_timer_cancel", "Cancel timers", "Cancel a previously scheduled timer.", "automation"
    ),
    ToolCatalogEntry(
        "sys_scheduled_task_create",
        "Create scheduled tasks",
        "Create a recurring agent run on a schedule (RRULE).",
        "automation",
    ),
    ToolCatalogEntry(
        "sys_scheduled_task_list",
        "List scheduled tasks",
        "List scheduled tasks with their schedules and state.",
        "automation",
    ),
    ToolCatalogEntry(
        "sys_scheduled_task_update",
        "Update scheduled tasks",
        "Update a scheduled task's mutable fields.",
        "automation",
    ),
    ToolCatalogEntry(
        "sys_scheduled_task_delete",
        "Delete scheduled tasks",
        "Delete a scheduled task so it no longer fires.",
        "automation",
    ),
    # Web & research
    ToolCatalogEntry(
        "web_search", "Web search", "Search the web for information.", "web_research"
    ),
    ToolCatalogEntry(
        "web_fetch", "Web fetch", "Fetch and extract content from a URL.", "web_research"
    ),
    ToolCatalogEntry(
        "nimble_research", "Nimble research", "Run a Nimble research task.", "web_research"
    ),
    ToolCatalogEntry(
        "nimble_extract",
        "Nimble extract",
        "Extract structured data from a URL via Nimble.",
        "web_research",
    ),
    ToolCatalogEntry(
        "browser_navigate",
        "Browser navigation",
        "Open or navigate the embedded browser to a URL.",
        "web_research",
    ),
    ToolCatalogEntry(
        "browser_snapshot",
        "Browser snapshot",
        "Capture an accessibility-tree snapshot of the browser.",
        "web_research",
    ),
    ToolCatalogEntry(
        "browser_click",
        "Browser click",
        "Click an element in the embedded browser.",
        "web_research",
    ),
    ToolCatalogEntry(
        "browser_type", "Browser type", "Type text into a browser input element.", "web_research"
    ),
    ToolCatalogEntry(
        "browser_screenshot",
        "Browser screenshot",
        "Capture a PNG screenshot of the browser pane.",
        "web_research",
    ),
    # Reviews
    ToolCatalogEntry(
    ...
    # Long-term memory
    ToolCatalogEntry(
        "hindsight_retain", "Hindsight retain", "Store information to long-term memory.", "memory"
    ),
    ToolCatalogEntry(
        "hindsight_recall",
        "Hindsight recall",
    ...
    ToolCatalogEntry(
        "hindsight_reflect",
        "Hindsight reflect",
        "Reflect on and consolidate long-term memory.",
        "memory",
    ),
    # MCP access
    ToolCatalogEntry(
        "mcp_tool_search",
        "Search MCP tools",
        "Search attached MCP servers for relevant tools.",
        "mcp",
    ),
    ToolCatalogEntry(
        "mcp_tool_call", "Call MCP tools", "Invoke an MCP tool returned by mcp_tool_search.", "mcp"
    ),
    # Model discovery
    ToolCatalogEntry(
        "sys_list_models",
        "List available models",
        "List models available to sub-agent workers.",
        "models",
    ),
    ToolCatalogEntry(
...
def get_catalog_response(disabled_tools: frozenset[str]) -> dict:
    ...
    return {
        ...
        "groups": [{"id": g.id, "title": g.title, "order": g.order} for g in GROUPS],
        "tools": [
            {
                "name": t.name,
                "title": t.title,
