...
def lookup_model_context_window(model: str) -> tuple[int | None, str | None]:
    """Return a known context window and its metadata source.

    Unlike :func:`get_model_context_window`, this function does not invent a
    fallback. Callers that surface confidence to users can therefore
    distinguish published metadata from an estimate.
    """
...
def get_model_context_window(model: str) -> int:
    ...
    """
    window, _source = lookup_model_context_window(model)
    return window if window is not None else _DEFAULT_CONTEXT_WINDOW
...
def fetch_provider_config_pricing(
    provider_config: dict[str, Any] | None,
    harness: str | None,
) -> ModelPricing | None:
    """Fetch custom pricing from provider config, or ``None`` when unconfigured.

    This is only the provider-config step of
    :func:`fetch_model_pricing_with_provider`, exposed so callers can slot
    other pricing sources (e.g. the OmniHarness model catalog) between
    provider-configured rates and the MLflow catalog.

    :param provider_config: Parsed provider config from
        :func:`~omnigent.onboarding.provider_config.load_config`.
    :param harness: Harness name used to pick the provider family; SDK
        executor spellings (``claude_sdk``, ``agents_sdk``) are normalized.
    :returns: Per-token :class:`ModelPricing` from the provider's configured
        rates, or ``None`` when unconfigured or when the lookup fails.
    """
    if provider_config is None or harness is None:
        return None
    ...
    try:
        ...
        # A malformed config or import error must not break cost tracking;
        # callers fall through to their next pricing source.
        pass
    return None
...
def fetch_model_pricing_with_provider(
    ...
    """
    pricing = fetch_provider_config_pricing(provider_config, harness)
