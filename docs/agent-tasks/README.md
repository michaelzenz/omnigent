...
* Broker: when router cannot pick the winner, surface to broker, which will route to an active task or create new task for the event, and also reconcile the event into taskItems
* Manager: Manager and task is 1:1 mapping, manager receives events, reconcile into taskItems and assign it to workers to run it
...
* TaskItem: TaskEvents got reconcile into TaskItem, for example multiple comment on same pr reconcile to same TaskItem, which is the execution unit containing instructions that actually get executed by workers.
