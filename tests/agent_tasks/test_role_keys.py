...
def test_system_roles_are_not_deletable() -> None:
    assert is_system_role_key(TASK_BROKER_ROLE_KEY)
    assert is_system_role_key(TASK_SECRETARY_ROLE_KEY)
    assert is_system_role_key(MANAGER_DEFAULT_ROLE_KEY)
    assert not is_deletable_role_key(TASK_BROKER_ROLE_KEY)
...
def test_normalize_rejects_unknown_roles() -> None:
    with pytest.raises(OmnigentError):
        normalize_role_profile_key("worker:default")
    with pytest.raises(OmnigentError):
        normalize_role_profile_key("adopted-session")
...
def test_role_kind_from_key_covers_every_family() -> None:
    ...
    assert role_kind_from_key("manager:research") == ROLE_KIND_MANAGER
...
def test_only_user_facing_roles_are_singletons() -> None:
    """Broker and secretary sessions are per-user; manager roles are per task."""
    ...
    assert not is_singleton_role_key("manager:research")
