...
async def test_delete_logs_reachable_worktree_failure_and_deletes_session(
    app: FastAPI,
    client: httpx.AsyncClient,
    db_uri: str,
) -> None:
    """A host-reported git error stays best-effort after the host responds."""
    ...
    assert resp.status_code == 200
    assert SqlAlchemyConversationStore(db_uri).get_conversation(conv_id) is None
