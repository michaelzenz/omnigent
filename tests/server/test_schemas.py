...
def test_session_git_auto_create_owns_branch_name() -> None:
    """Auto mode accepts a base ref but does not accept a client branch."""
    from omnigent.server.schemas import SessionCreateRequest, SessionGitOptions
    ...
    req = SessionCreateRequest(
        ...
        host_id="host_abc",
        workspace="/repo",
        git=SessionGitOptions(auto_create=True, base_branch="main"),
    )
    assert req.git is not None
    assert req.git.auto_create is True
    assert req.git.branch_name is None

    with pytest.raises(
        ValidationError,
        match="branch_name is generated by the server when auto_create is true",
    ):
        SessionGitOptions(auto_create=True, branch_name="client-owned")
