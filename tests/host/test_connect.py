...
def test_enrich_path_sources_interactive_rc(
    ...
    rc_file.write_text('export PATH="/opt/homebrew/bin:$PATH"\n')
    ...
    assert "/opt/homebrew/bin" in enriched, f"expected /opt/homebrew/bin in PATH, got {enriched!r}"


def test_enrich_path_noop_when_rc_missing(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_enrich_path_falls_back_to_profile(
    ...
    profile.write_text('export PATH="/custom/bin:$PATH"\n')
