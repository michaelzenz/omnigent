...
def test_profile_state_filters_disabled_agents(agent_store: SqlAlchemyAgentStore) -> None:
    agent = agent_store.create(
        agent_id="d4" * 16,
    ...
    assert agent.id in {row.id for row in agent_store.list(limit=100, include_disabled=True).data}
