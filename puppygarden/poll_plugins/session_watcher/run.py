#!/usr/bin/env python3
"""Poll plugin: session_watcher — discover and track external sessions.

Scans configured directories for session transcript files, detects new
sessions and transcript changes (including rewinds), and posts events to
the server. Uses incremental history hashes for robust rewind detection.

State is persisted in ``state.yaml`` alongside ``run.py``.
"""
...
def parse_transcript(path: Path) -> list[str]:
    """Parse a session file into a list of message strings.

    Handles JSONL (one JSON object per line) and plain text. Each JSON
    line with a ``role`` field is treated as a message boundary.
    """
    try:
...
def compute_history_hashes(messages: list[str]) -> list[str]:
    """Compute chained hashes: h[i] = hash(h[i-1] || message[i])."""
    hashes: list[str] = []
    prev = ""
    for msg in messages:
...
def find_divergence(
    ...
    """Return the first index where current diverges from stored."""
...
def post_update(
    ...
    """Post a transcript update. Returns ``track`` from the server."""
    import httpx
    ...
    resp = httpx.post(
        f"{base}/v1/session-watcher/update",
        headers=headers,
        json=body,
        timeout=30.0,
    )
...
def find_session_files(scan_dirs: list[Path]) -> list[Path]:
    """Find candidate session files in scan directories."""
    results: list[Path] = []
    for d in scan_dirs:
        if not d.exists():
            continue
...
def main() -> None:
    ...
    for fpath in files:
        ...
        # Check recency — skip sessions older than the window
        try:
        ...
        # Check sink time — skip sessions active within the window
