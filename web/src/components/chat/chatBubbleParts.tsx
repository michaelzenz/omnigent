...
import type { ChatTopButtonMode } from "@/lib/chatTopButtonPreferences";
...
export function JumpToTopButton({
  ...
  const jumpToPreviousMessage = useCallback(async () => {
    ...
    const findTarget = (): {
      ...
      const index = userMessageIndexNearestRoof(
        messages.map((message) => ({ top: naturalElementTop(message) - naturalElementTop(el) })),
        roof,
      );
      ...
      return {
        messages,
        index,
        atRoof: Math.abs(messages[index].getBoundingClientRect().top - bannerBottom) <= 1,
      };
    };
