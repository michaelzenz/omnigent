...
import type { NativeModelOption, Session, SessionStatus, WorktreeStatus } from "@/lib/types";
...
export function ChatPage() {
  ...
  const onSend = useCallback(
    (text: string, files?: File[]) => {
      ...
      void chat.send(text, agentId, files, {
        onConversationCreated: (newId) => navigate(`/c/${newId}`, { replace: true }),
      });
    },
    [
      agentId,
  ...
  // Once present, the live session snapshot is authoritative. Memoized so
  // derived controls keep stable identities across switch re-renders.
 ...
 */
export const MainAgentSurface = memo(function MainAgentSurfaceImpl({
  conversationId,
  conversationKey,
  status,
  isWorking,
...
export function WorktreeCreationPanel({
  ...
  const keyedLogLines = logLines.map((line) => {
...
export function effortLevelsForConv(
  ...
  switch (conv?.labels?.["omnigent.wrapper"]) {
    ...
    default:
      if (isLabelLessCodexNative(conv))
        return codexEffortLevelsForModel(codexModelOptions, currentModel);
  ...
  switch (conv?.labels?.["omnigent.wrapper"]) {
    ...
    default:
      return isLabelLessCodexNative(conv)
        ? "codex"
        : isOnihTargetName(conv?.agentName)
          ? "sdk"
          : null;
  }
}
