 ...
 * @param options - Optional title, agent switch, truncation point,
 *   managed-worktree request, and run-config overrides. `agentId` can
 *   switch the fork to a built-in agent (e.g.
 ...
 *   `upToResponseId` is an optional truncation point, e.g. "resp_abc". When
 *   set, the fork copies history only up to and including that response
 *   ("fork from here"); omitted, the full history is copied.
 *   Run-config fields are optional overrides from the fork dialog's
...
export async function forkSession(
  ...
  if (options.terminalLaunchArgs !== undefined) {
    body.terminal_launch_args = options.terminalLaunchArgs;
  }
