...
const AGENT_PICKER_DESCRIPTIONS: Record<string, string> = {
  "onih-openai-agents": "Onih with OpenAI Agents",
  "onih-pi": "Onih with Pi",
  polly: "Multi-agent coding",
  debby: "Multi-agent debate",
};

export function isNewSessionHarnessAgent(agent: AvailableAgent): boolean {
  return isNativeCodingAgent(agent) || isAcpHarnessAgent(agent) || isOnihTargetName(agent.name);
}
...
function HarnessConfigModal({
  ...
  const isOmniHarness = isOnihTargetName(agent.name);
  const hasSdkModelControl =
...
export function NewChatLandingScreen() {
  ...
  const { agentList, agentEntries, harnessEntries } = useMemo(
    () => groupNewSessionAgents(selectableSessionAgents(agents ?? [])),
    [agents],
  );

  // Split the picker into "Harnesses" (native terminal CLIs, generic ACP
  ...
  const omniharnessBaseAgent = harnessEntries.find((agent) => isOnihTargetName(agent.name));
  ...
  const selectedAgentUsesOmniHarness = isOnihTargetName(selectedAgent?.name);
  ...
  const projectModelVocab = selectedAgentUsesOmniHarness
    ? sdkModelOptions
  ...
  useEffect(() => {
    ...
    const projectSeed =
      !userPickedModelRef.current &&
      projectDefaultModel != null &&
      sdkModelOptions.some((model) => model.id === projectDefaultModel)
        ? projectDefaultModel
        : null;
    setPickedModel(
      projectSeed ??
  ...
  }, [isGitWorkspace, autoWorktreesSupported, currentWorkspaceBranch, autoBaseBranchEdited]);
  ...
  const handleSelectAgent = (agent: AvailableAgent) => {
    setSmartRoutingDropped(null);
    if (isOnihTargetName(agent.name)) {
  ...
  async function handleCreate() {
    ...
    try {
      // `canSubmit` requires a resolved agent; capture the narrowed invariant
      // for the async create flow.
      ...
      const agentSupportsModelOverride =
        agentSupportsModelPicker ||
        nativeAgent?.harness === "codex-native" ||
        isOnihTargetName(agent.name);
      ...
      if (effectiveAgentId === PENDING_AGENT_ID && pendingAgent) {
        ...
        const createRequest = authenticatedFetch("/v1/sessions", {
          ...
          body: JSON.stringify({
            ...
            ...(agentFromProjectConfig ? {} : { agent_id: sessionAgentId }),
      ...
      if (onScreenRef.current) {
        ...
        void useChatStore
          .getState()
          .bindBackgroundSession(data.id, sessionAgentId, sessionAgent?.display_name ?? null);
      }
    } catch {
      returnDraftToUser();
              ...
              if (worktreeSizes?.error) {
                return (
                  <div className="flex items-center gap-2 px-2 pb-1">
                    ...
                    <Dialog
                      open={worktreeSizePopoverOpen}
                      onOpenChange={setWorktreeSizePopoverOpen}
                    >
                  ...
                  <>
                    <div className="flex items-center gap-2 px-2 pb-1">
                      <button
                        ...
                        data-testid="new-chat-worktree-sizes-summary"
                      >
                        {worktreeSizes.data.length} worktree
                        {worktreeSizes.data.length > 1 ? "s" : ""}
                        {" · "}
                      ...
                      <DialogContent className="sm:max-w-[53rem]">
                        <DialogHeader>
                          ...
                          {worktreeSizes.calculated_at > 0 && (
                            <DialogDescription>
                              Updated{" "}
                              {Math.round((Date.now() / 1000 - worktreeSizes.calculated_at) / 60)}{" "}
                              min ago
                            </DialogDescription>
                          )}
                        </DialogHeader>
                          ...
                          {worktreeSizes.data.map((wt) => (
                            ...
                            >
                              ...
                              {wt.error ? (
                                <>
                                  <span
                                    className="text-center text-xs text-amber-500"
                                    title={wt.error}
                                  >
                                    ⚠ error
                                  </span>
                                  <span className="text-center text-xs text-muted-foreground/40">
                                    —
                                  </span>
                                  <span className="text-right text-xs text-muted-foreground/40">
                                    —
                                  </span>
                                </>
                              ) : (
