...
const pendingForegroundBinds = new Map<string, ConversationEntry>();
let pendingSeq = 0;
let queueSeq = 0;
...
export const useChatStore = create<ChatState>((_rootSet, get) => ({
  ...
  switchTo: async (conversationId) => {
    ...
    pendingForegroundBinds.set(conversationId, entry);
    ...
    try {
      ...
      if (pendingForegroundBinds.get(conversationId) === entry) {
  ...
  submitApproval: async (elicitationId, action, content, meta) => {
    ...
    try {
      const resp = await approveElicitation(targetSessionId, elicitationId, {
        action,
        ...(content === undefined ? {} : { content }),
        ...(meta === undefined ? {} : { _meta: meta }),
      });
